...
class DataRetriever:
    def __init__(self, model_name, url=None, system_prompt="", examples=""):
        ...
        # Use structured data for the generation of the output by the LLM.
        self.llm = self.llm.with_structured_output(DataFields)
        # Create the whole system prompt including the examples as well.
        examples = "\n\n".join(examples)
        self.system_prompt = f"{system_prompt}\n\n{examples}"
        prompt = ChatPromptTemplate.from_messages([("system", system_prompt), ("human", "{input}")])

        # Create the overall pipeline made of prompt and llm
